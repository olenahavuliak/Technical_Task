...
public class Main {
  ...
  private static Product getMostPopularProduct(List<Order> orders) {
    ...
    int maxValueInMap = Collections.max(productMap.values());
  ...
  private static double calculateAverageAge(Product product, List<Order> orders) {
    ...
    for(Order order: orders){
        if(order.getProductList().contains(product)){
          avgAge+= order.getUser().getAge();
          userCount++;
        }
      }
    if(userCount!= 0 && avgAge!=0)
      return (double) avgAge/userCount;
    return 0;
  }
  ...
  private static List<Product> sortProductsByPrice(List<Product> products) {
    List <Product> sortedProductList = new ArrayList<>(products);
    sortedProductList.sort(Comparator.comparingDouble(Product::getPrice));
    return sortedProductList;
  }

  private static List<Order> sortOrdersByUserAgeDesc(List<Order> orders) {

    List<Order> sortedOrderList = new ArrayList<>(orders);
    sortedOrderList.sort(Comparator.comparingDouble(order-> order.getUser().getAge()));
      ...
      for(Order order: orders){
        ...
        for(Product product: productList){
          if(product instanceof RealProduct){
            ...
            weight+=realProduct.getWeight();
          }
        }
        orderMap.put(order, weight);
      }
    return orderMap;
  }
}